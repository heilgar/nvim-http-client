...
function M.display_debug_info(http_request)
    local env_file = http_request.environment.get_current_env_file() or "Not set"
    local env = vim.inspect(http_request.environment.get_current_env() or {})
    local current_request = vim.inspect(http_request.http_client.get_current_request() or {})

    local request_under_cursor = vim.inspect(http_request.parser.get_request_under_cursor() or {})
